...
var web2spa = {
  ...
  */
      ...
      var path = '%s/%s'.format(this.root,opts.ctrl||this.mainctrl);
      if (opts.error_path) opts.master = true;
      if (!opts.master) path += '/%s'.format(opts.page||this.mainpage);
      var route = (this.mega ? path : this.root) + ':' + ctrl;
      ...
      if (opts.login_path) this.login_path = '/%s/%s/'.format(path,ctrl).normPath();
      if (opts.error_path) this.error_path = '/%s/%s/'.format(path,ctrl).normPath();
      if (opts.shortcuts) {
          path = path.split('/');
          title = '';
